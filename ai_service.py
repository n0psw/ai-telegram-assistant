...
async def check_intent(user_query: str) -> dict:
    ...
    """
    router_prompt = """Ты — умный помощник для поиска по базе магазина «Центр Красок #1».
Твоя задача — формировать поисковые запросы.
ПРАВИЛО: Ставь "is_relevant": true ПОЧТИ ВСЕГДА. 
Блокируй (is_relevant: false) ТОЛЬКО явный спам, оскорбления, рецепты еды, программирование или политику.
ЛЮБЫЕ вопросы про людей, компании, адреса, цифры, советы, помощь — пропускай (true), так как это может быть в нашей базе.
Ответь СТРОГО в формате JSON:
{
  "is_relevant": boolean,
  "search_query": "короткая суть запроса для поиска (если is_relevant=false, то null)"
}"""
